...
if getattr(sys, "frozen", False):
    # PyInstaller 打包运行：临时解压目录不可写持久，数据库/答案缓存必须放在 exe 旁边
    script_dir = os.path.dirname(os.path.abspath(sys.executable))
os.chdir(script_dir)
DB_PATH = os.path.join(script_dir, "database.db")
COURSE_ANSWERS_PATH = os.path.join(script_dir, "course_answers.json")
# 首次运行时把内置的题库与答案缓存复制到 exe 旁，之后写入可持久化
if getattr(sys, "frozen", False):
    for bundled, real in ((os.path.join(sys._MEIPASS, "database.db"), DB_PATH),
                          (os.path.join(sys._MEIPASS, "course_answers.json"), COURSE_ANSWERS_PATH)):
        if not os.path.exists(real) and os.path.exists(bundled):
            import shutil
            shutil.copy(bundled, real)
            print("已初始化数据文件：", real)
